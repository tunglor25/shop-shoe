        ...
        <div class="offcanvas-body">
            @if (1>0)
                @foreach ($cartItems as $item)
                    <div class="cart-item mb-3 border-bottom pb-2">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="d-flex align-items-center">
                                <form action="{{ route('cart.update', $item->user_id) }}" method="POST"
                                    class="d-flex align-items-center me-2">
                                    @csrf
                                    @method('PUT')
                                    <input type="number" name="quantity" value="{{ $item->qty }}"
                                        min="1" class="form-control form-control-sm w-50">
                                    <button class="btn btn-sm btn-outline-success ms-1" type="submit"><i
                                            class="fas fa-sync"></i></button>
                                </form>
                                <form action="{{ route('cart.remove', $item->user_id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                ...
                <div class="text-end mt-3">
                    ...
                    <a href="{{ route('checkout') }}" class="btn btn-primary w-100 mt-3">Mua ngay</a>
                </div>
            @else
