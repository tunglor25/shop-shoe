...
@section('content')
    ...
    <div class="container-fluid py-4">
        ...
        <nav aria-label="breadcrumb">
            <div class="d-flex justify-content-between align-items-center">
                <ol class="breadcrumb mb-0 bg-transparent">
                    <li class="breadcrumb-item"><a href="#" class="text-gold-soft"><i class="fas fa-home me-1"></i>
                            Home</a></li>
        ...
        <h1 class="h2 mt-4 mb-4 text-gold" style="font-family: 'Playfair Display', serif;">
            <i class="fas fa-newspaper me-2"></i> {{ $title }}
        </h1>
                ...
                <div class="luxury-card mb-4"
                    ...
                    <div class="card-header py-3" style="border-bottom: 1px solid rgba(212,175,55,0.3);">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('admin.news.create') }}" class="btn btn-gold btn-sm">
                            ...
                            <table class="table mb-0"
                                ...
                                <thead>
                                    <tr style="background: rgba(212,175,55,0.1);">
                                        ...
                                        <th class="text-gold border-gold">TIÊU ĐỀ</th>
                                        <th class="text-gold border-gold">ẢNH ĐẠI DIỆN</th>
                                        <th class="text-gold border-gold">DANH MỤC</th>
                                        <th class="text-gold border-gold">NGÀY ĐĂNG</th>
                                        <th class="text-gold border-gold">TRẠNG THÁI</th>
                                        <th class="text-gold border-gold">NỔI BẬT</th>
                                ...
                                <tbody>
                                    @foreach ($news as $item)
                                        <tr style="border-bottom: 1px solid rgba(212,175,55,0.1);">
                                            <td class="text-light border-gold">{{ $item->id }}</td>
                                            <td class="text-light border-gold">
                                                <strong>{{ $item->title }}</strong>
                                                <div class="text-gold-soft small mt-1">{{ Str::limit($item->summary, 50) }}
                                                </div>
                                            </td>
                                            <td class="border-gold">
                                                @if ($item->image)
                                                    <img src="{{ asset('storage/' . $item->image) }}" width="60"
                                                        height="60" class="rounded border border-gold"
                                                        style="object-fit: cover;">
                                                @else
                                                    <div class="bg-dark rounded border border-gold d-flex align-items-center justify-content-center"
                                                        style="width:60px;height:60px;">
                                                        <i class="fas fa-image text-gold-soft"></i>
                                                    </div>
                                                @endif
                                            </td>
                                            <td class="text-light border-gold">{{ $item->category->name ?? '-' }}</td>
                                            <td class="text-light border-gold">
                                                {{ $item->published_at}}</td>
                                            <td class="border-gold">
                                                @if ($item->status)
                                            ...
                                            <td class="border-gold">
                                                @if ($item->is_featured)
                                                    <span class="badge bg-gold text-dark">NỔI BẬT</span>
                                                @else
