                ...
                <div class="card-body">
                    <div class="table-responsive">
                                ...
                                @foreach ($product->getAttributes() as $field => $value)
                                    <tr style="border-bottom: 1px solid rgba(212,175,55,0.1);">
                                        ...
                                        <td class="text-light border-gold">
                                            @switch($field)
                                                @case('image')
                                                    <img src="{{ asset('storage/' . $value) }}" width="150" class="rounded border border-gold" style="object-fit: cover;">
                                                @break
                                                ...
                                                @case('category_id')
                                                    <span class="text-gold">{{ $product->category->name }}</span>
                                                @break

                                                @case('price')
                                                    <span class="text-gold">{{ number_format($value) }}đ</span>
                                                @break

                                                @case('description')
                                                    <div class="text-light" style="max-height: 200px; overflow-y: auto;">
                                                        {!! $value !!}
                                                    </div>
                                                @break
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            ...
                            <div class="btn-group">
                                <a href="{{ route('admin.products.edit', $product->id) }}" class="btn btn-gold me-2">
                                    <i class="fas fa-edit me-2"></i> CHỈNH SỬA
                                </a>
                                <form action="{{ route('admin.products.destroy', $product->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-outline-danger" onclick="return confirm('Bạn chắc chắn muốn xóa sản phẩm này?')">
